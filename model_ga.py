...
class GeneticAlgorithm:
    ...
    def evaluate_fitness(self, architecture, train_loader, val_loader, device, epochs=100):
        ...
        try:
            ...
            # Calculate separate parameter counts for Conv and FC layers
            conv_params = 0
            fc_params = 0
            ...
            conv_params_normalized = conv_params / 1e6
            fc_params_normalized = fc_params / 1e6

            # Weight justification:
            # Conv layers: weight = 2.0
            #   - Convolution operations are computationally intensive (O(K²×C×H×W) FLOPs)
            #   - Involve spatial operations across entire feature maps
            #   - Higher memory bandwidth requirements due to feature map storage
            #   - Dominate inference time in CNNs
            #
            # FC layers: weight = 0.5
            #   - Simple matrix multiplication (O(M×N) FLOPs)
            #   - No spatial processing overhead
            #   - Typically processed efficiently with optimized BLAS libraries
            #   - Usually smaller contribution to total compute in modern CNNs

            conv_weight = 2.0
            fc_weight = 0.5

            # Calculate weighted complexity penalty
            complexity_penalty = (conv_weight * conv_params_normalized +
                                  fc_weight * fc_params_normalized)

            del model, inputs, outputs, labels
            torch.cuda.empty_cache()

            # Fitness = accuracy - lambda * weighted_complexity
            ...
            print(f"Conv params: {conv_params:,}, FC params: {fc_params:,}, "
                  f"Penalty: {complexity_penalty:.4f}", flush=True)
    ...
    # def evaluate_fitness(self, architecture, train_loader, val_loader, device, epochs=100):
    #     """ OLD - Train and evaluate a single architecture"""
    #     try:
    #         model = CNN(architecture.genes).to(device)
    #         criterion = nn.CrossEntropyLoss()
    #         optimizer = AdamW(model.parameters(), lr=0.001)
    ...
    def selection(self):
        ...
        print(f"Performing Roulette Wheel Selection ...", flush=True)
        total_fitness = sum(
            currentSelection.fitness for currentSelection in self.population)
        relative_fitness = [currentSelection.fitness /
                            total_fitness for currentSelection in self.population]
        probs = [sum(relative_fitness[:i+1])
    ...
    def crossover(self, parent1, parent2):
        ...
        min_len = min(child1_genes['num_conv'],
                      len(child1_genes['conv_configs']))
        ...
        min_len = min(child2_genes['num_conv'],
                      len(child2_genes['conv_configs']))
    ...
    def mutation(self, architecture):
        ...
        mutation_type = random.choice(
            ['conv_param', 'num_layers', 'pool_activation', 'fc_units'])
        ...
        if mutation_type == 'conv_param' and genes['conv_configs']:
            ...
            genes['conv_configs'][idx]['filters'] = random.choice(
                self.search_space.filters)
            genes['conv_configs'][idx]['kernel_size'] = random.choice(
    ...
    def evolve(self, train_loader, val_loader, device, run=1):
        ...
        print(
            f"Starting with {self.population_size} Population:\n{self.population}\n", flush=True)

        for generation in range(self.generations):
            print(f"\n{'='*60}", flush=True)
            print(
                f"Generation {generation + 1}/{self.generations}", flush=True)
            print(f"{'='*60}", flush=True)

            # Evaluate fitness
            for i, arch in enumerate(self.population):
                print(
                    f"Evaluating architecture {i+1}/{self.population_size}...", end=' ', flush=True)
                fitness = self.evaluate_fitness(
                    arch, train_loader, val_loader, device)
                print(
                    f"Fitness: {fitness:.4f}, Accuracy: {arch.accuracy:.4f}", flush=True)
            ...
            next_generation.extend(
                [deepcopy(self.population[0]), deepcopy(self.population[1])])
